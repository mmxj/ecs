<template>
    <div>
        <div class="input-group">
            <input v-model="CustomerName" value="" id="txtCustomerName" type="text" maxlength="100" class="form-control required" data-valid="isNonEmpty" data-error="客户名称不能为空" placeholder="请选择所属客户" disabled="disabled"/>
            ...
            <input type="hidden" v-model="CustomerId" id="hidCustomerId" value=""/>
        </div>
        <div id="modal-cleosCompany" class="modal fade" tabindex="-1" role="dialog" data-backdrop="static" aria-hidden="true" style="display: none;">
                <div class="modal-content">
                    <div class="modal-header">
                        <button type="button" @click="modalClose()" class="win-close" data-dismiss="modal">×</button>
                        <button id="btnSave" type="button" class="sava" data-dismiss="modal" aria-hidden="true" v-on:click="get_tempUser()">
                    ...
                    <div class="modal-body">
                        <div>
                            <span class="OrgTopAligin">
                                <input id="customerSearchTxt" v-model.trim="customerSerach" class="form-control" placeholder="客户编号" type="text">
                            </span>
                            <span class="OrgTopAligin">
                        ...
                        <div class="dataTables_wrapper no-footer mt-10">
                            ...
                            <table class="table table-striped table-bordered dataTable no-footer employee_table" role="grid" aria-describedby="example1_info" style="width: 100%;" width="100%">
                                ...
                                <tr role="row">
                                    ...
                                    <th rowspan="1" colspan="1">客户编号</th>
                                    <th rowspan="1" colspan="1">客户名称</th>
                                </tr>
                                </thead>
                                <tbody>
                                <tr v-bind:class="((index % 2) == 0 ? 'odd' : 'even') + (selectRow == item.CustomerId ? ' selected' : '')" v-on:click="is_check_tr(item)" role="row" v-for="(item,index) in employeeListData" @click="ischeckdate(item)">
...
<script>
    export default{
        ...
        methods: {
            getData: function() {
                ...
                var params = {
                    ...
                    "Parameters": {
                        "Keywords": this.customerSerach,
                    }
                };
                eosCommon.eosAjax(eosCommon.COMMON_API + 'api/common/customer', "GET", params, "json", function(result) {
                    _this.processing = 1;
            ...
            newGetData: function() {
                this.selectRow = $("#hidCustomerId").val();
                this.customerSerach = '';
                this.CustomerName = $("#CustomerName").val();
                this.CustomerId = $("#hidCustomerId").val();
                this.page_index = 1;
                this.getData();
            },
            serarch: function() {
            ...
            is_check_tr: function(item) {
                this.selectRow = item.CustomerId;
            },
            get_tempUser: function() {
                if(this.customerData == '') {
                    ...
                    $('#txtCustomerName').next('.valid-form-group-addon').find('.focus').html('');
                    $('#txtCustomerName').attr('isSelected', '1');
                    $('.select_tips').css('display', 'none');
                }
            },
            modalClose: function() {
                $('#txtCustomerName').val('');
        ...
        mounted: function() {
            var _this = this;
            this.getData();
            $('#customerSearchTxt').bind('keypress', function(event) {
