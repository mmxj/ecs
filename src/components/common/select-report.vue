<template>
	<div>
		<span class="input-group">
            <input v-model="ReportToName" id="txtReportToName" type="text" maxlength="100" class="form-control" placeholder="请选择汇报人" disabled="disabled" />
            <span class="input-group-addon" data-toggle="modal" data-target="#modal-report" @click="newGetData()"><span class="fa fa-bars"></span></span>
            <input v-model="ReportTo" type="hidden" id="hidReportToId" value="" />
        </span>
		<div id="modal-report" class="modal fade" tabindex="-1" role="dialog" data-backdrop="static" aria-labelledby="myModalLabel" aria-hidden="true" style="display: none;">
		    <div class="modal-content">
		        <div class="modal-header">
		            ...
		            <h4 class="modal-title">汇报人</h4>
		        </div>
		        <div id="verifyCheck">
		            <div id="modal_body1" class="modal-body nicescroll">
		                <div class="">
		                    <span class="OrgTopAligin">
		                        <input id="reportSearchTxt" v-model.trim="employeeSerach" class="form-control" placeholder="工号/姓名" type="text">
		                    </span>
		                    <span class="OrgTopAligin">
...
<script>
	export default{
	    ...
	    methods: {
	        ...
	        newGetData: function (){
	        	this.selectRow = $("#hidReportToId").val();
        	    this.employeeSerach = '';
    	    	this.ReportToName = $("#txtReportToName").val();
    	        this.ReportTo  = $("#hidReportToId").val();
    	        this.page_index  = 1;
	        	this.getData();
	        },
	        serarch: function () {
	        ...
	        ischeckdate: function (item) {
	            this.ReportToData = item;
	        },
	        is_check_tr: function (item) {
	            this.selectRow = item.id;
	        },
	        get_report: function () {
	            if (this.ReportToData == '') {
	            	return false;
	            }else{
	            	this.ReportToName = this.ReportToData.name;
	                this.ReportTo   = this.ReportToData.id;
	            }
	        },
	        up_page: function () {
	    ...
	    mounted: function() {
	    	var _this = this;
	        this.getData();
	        $('#reportSearchTxt').bind('keypress',function(event){
                if(event.keyCode == "13"){
                    _this.getData();
                }
            });
