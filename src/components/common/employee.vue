<template>
	<div>
		<div id="modal-employee" class="modal fade" tabindex="-1" role="dialog" data-backdrop="static" aria-labelledby="myModalLabel" aria-hidden="true" style="display: none;">
		        ...
		        <div class="modal-body">
		            <div class="">
		                <span class="OrgTopAligin">
		                    <input v-model.trim="serarchTxt" class="form-control" placeholder="工号/姓名" type="text">
		                </span>
		                <span class="OrgTopAligin">
		            ...
		            <div class="dataTables_wrapper mt-10">
		                <table class="table table-striped table-bordered dataTable no-footer employee_table" role="grid" aria-describedby="example1_info" style="width: 100%;" width="100%">
		                    ...
		                    <tbody>
		                        <tr v-bind:class="((index % 2) == 0 ? 'odd' : 'even') + (selectRow == index ? ' selected' : '')" v-on:click="is_check_tr(index)" role="row" v-for="(item,index) in employeeListData" @click="ischeckdate(item)">
	    ...
	    methods: {
	        getData_isAcount: function () {
	            ...
	            var param = {
	                ...
	                "Parameters": {
	                    "Keywords": this.serarchTxt
	                }
	            }
	            eosCommon.eosAjax(eosCommon.COMMON_API + "api/common/employee" , "GET", param, "json", function (result) {
	                _this.processing = 1;
	        ...
	        getData_notAcount: function () {
	            ...
	            var param = {
                    ...
                    var url = eosCommon.COMMON_API + "api/common/employeedetail";
	        ...
	        serarch: function () {
	             if(this.isHasAccount=='0'){
    	        	this.getData_notAcount();
    	        	
    	        }else if(this.isHasAccount=='1'){
	        ...
	        get_employeeId: function () {
	            if (this.employeeData == '') {
	                ...
	                this.data.id = this.employeeData.id;
	                this.data.name = this.employeeData.name;
	                this.data.UserId = this.employeeData.UserId;
