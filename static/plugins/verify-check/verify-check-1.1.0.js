...
(function ($) {
	...
	function require(options){
		var rule={//验证规则
			...
			company:/^[\u4E00-\u9FA5a-zA-Z][\u4E00-\u9FA5a-zA-Z0-9\s-,_]*$/,
			uname:/^[\u4E00-\u9FA5a-zA-Z][\u4E00-\u9FA5a-zA-Z0-9_]*$/,
			OtherName:/^[\u4E00-\u9FA5a-zA-Z0-9][\u4E00-\u9FA5a-zA-Z0-9_\s-,-.-\/]*$/,
			computerphobe:/^[a-zA-Z0-9][a-zA-Z0-9_\s-,-.]*$/,
			zh:/^[\u4e00-\u9fa5]+$/,//纯中文
			zhEn:/^[\u4e00-\u9fa5a-zA-Z（）]+$/,//中英文  ^[A-Za-z0-9\u4e00-\u9fa5]+$
			zhEnNum:/^[\u4e00-\u9fa5_a-zA-Z0-9_]{2,10}$/,
			//zhEnNum:/^[\u4e00-\u9fa5_a-zA-Z0-9_]{2,10}$/,
			...
			en:/^[a-zA-Z_\s-,-.-\/]*$/,
			numFloatTwo:/^-?\d+\.?\d{0,2}$/,
			numFloatFour:/^-?\d+\.?\d{0,4}$/,
			numFloatSix:/^-?\d+\.?\d{0,6}$/,
			s:'',
			email: /^\w+((-\w+)|(\.\w+))*\@[A-Za-z0-9]+((\.|-)[A-Za-z0-9]+)*\.[A-Za-z0-9]+$/,
            tel:/^((\d{3,4}\-)|)\d{7,8}(|([-\u8f6c]{1}\d{1,5}))$/,
            ipv4: /((?:(?:25[0-5]|2[0-4]\d|((1\d{2})|([1-9]?\d)))\.){3}(?:25[0-5]|2[0-4]\d|((1\d{2})|([1-9]?\d))))/,
            oneToHundred: /(^[1-9][0-9]$)|(^100&)|(^[1-9]$)$/,
            oneToHundredTwentyFive: /(^[1][0-2][0-5]$)|(^[1-9][0-9]$)|(^100&)|(^[1-9]$)$/,
            oneToFiveThounsand: /(^[1-5][0][0][0]$)|(^[1-4][0-9][0-9][0-9]$)|(^[1-9][0-9][0-9]$)|(^[1-9][0-9]$)|(^100&)|(^[1-9]$)$/
		};
		this.rules={
			isCanEmpty: function(value, errorMsg) {//可以为空
				errorMsg=errorMsg||" ";
			},
			...
			isPrice: function(value, errorMsg) {
				errorMsg=errorMsg||" ";
				if (!rule.price.test(value)) return errorMsg;
			},
			isUname: function(value, errorMsg) {
				errorMsg=errorMsg||" ";
				if (!rule.uname.test(value)) return errorMsg;
			},
			isOtherName: function(value, errorMsg) {
				errorMsg=errorMsg||" ";
				if (!rule.OtherName.test(value)) return errorMsg;
			},
			isComputerphobe: function(value, errorMsg) {
				errorMsg=errorMsg||" ";
				if (!rule.computerphobe.test(value)) return errorMsg;
			},
			...
			isZhEn: function(value, errorMsg) {
				errorMsg=errorMsg||" ";
				if (!rule.zhEn.test(value)) return errorMsg;
			},
			isZhEnNum: function(value, errorMsg) {
				errorMsg=errorMsg||" ";
				if (!rule.zhEnNum.test(value)) return errorMsg;
			},
			isCard: function(value, errorMsg) {
				errorMsg=errorMsg||" ";
				if (!rule.card.test(value)) return errorMsg;
			},
			isNumFloatTwo: function(value, errorMsg) {
				errorMsg=errorMsg||" ";
				if (!rule.numFloatTwo.test(value)) return errorMsg;
			},
			isNumFloatFour: function(value, errorMsg) {
				errorMsg=errorMsg||" ";
				if (!rule.numFloatFour.test(value)) return errorMsg;
			},
            isNumFloatSix: function(value, errorMsg) {
				errorMsg=errorMsg||" ";
				if (!rule.numFloatSix.test(value)) return errorMsg;
			},
            ...
            isIpv4: function(value, errorMsg) {
                errorMsg=errorMsg||" ";
                if (!rule.ipv4.test(value))  return errorMsg;
            },
            isOneToHundred: function(value, errorMsg) {
                errorMsg=errorMsg||" ";
                if (!rule.oneToHundred.test(value))  return errorMsg;
            },
            isOneToHundredTwentyFive: function(value, errorMsg) {
                errorMsg=errorMsg||" ";
                if (!rule.oneToHundredTwentyFive.test(value))  return errorMsg;
            },
            isOneToFiveThounsand:function(value, errorMsg){
            	 errorMsg=errorMsg||" ";
                if (!rule.oneToFiveThounsand.test(value))  return errorMsg;
            }
		}
	};
	require.prototype={
	...
	*/
	verifyCheck._click=function(formId){
		...
		if(elArr[0]){
			$(window).scrollTop(elArr[0][0]['offsetParent']['offsetTop']);
		}
